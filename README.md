...
- 首页金额卡片改用绣球花猫咪背景，图片不透明度为 50%，并清理底部猫咪脸旁的黑色方块。
- 动态金额与背景分层，文字带淡底，长金额自适应字号、极长金额允许换行。
- 原金额计算、账单和备份格式保持不变；按最小必要验证原则未运行 lint/build/测试。

## 1.3.0 更新

- 「宠物」改名为「金丝熊」，图标换成蝴蝶结猫；交通图标换成飞机猫。
- 底部首页、账单、统计分别使用三只厨师猫，设置图标保持不变。
- 保留原分类 ID、已有账单和 JSON 备份格式；本次按最小必要验证原则未运行 lint/build/测试。
