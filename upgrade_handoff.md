...
- 此段写入时处于发布准备阶段；提交、推送及 Pages 部署结果将在核实后补记，不能仅根据版本号推断部署成功。
...
- `1.1.0` 发布时新增支出可选分类为：餐饮、超市、交通、购物、日用品、宠物、其他，共 7 项；随后本地最新需求进一步精简为 5 项，见本节开头。所有入口共用 `TransactionEditor`，因此「记一笔」的选项一致。
...
| 快速记账 | 底部弹层，突出金额，支出/收入、分类、日期和备注；本地新增支出 5 项分类，均使用猫咪图标；编辑旧账单时可保留其停用分类 |
...
用户此前指出：「我看到你加了 8 张图，但是我在 app 里只看到 3 张吧？」当时实际只有 3 种图案。`1.1.0` 新增宠物皇冠猫后共 4 种；后续本地加入四只分类猫后共 8 种。不能把尺寸变体或母版计为新的表情。
...
| 吃面猫（本地新增） | 餐饮分类的记账选项、账单行及统计图例 | cat-food-master.png + cat-food.png |
| 乘车猫（本地新增） | 交通分类的记账选项、账单行及统计图例 | cat-transport-master.png + cat-transport.png |
| 纸巾猫（本地新增） | 日用品分类的记账选项、账单行及统计图例 | cat-daily-master.png + cat-daily.png |
| 思考猫（本地新增） | 其他支出分类的记账选项、账单行及统计图例 | cat-other-master.png + cat-other.png |

本地合计 19 个图片文件：8 张母版、4 个 PWA/Apple 图标文件、2 张 WebP 插图、5 张分类 PNG。两张 WebP 插图主要用于空状态和设置；分类猫咪在记账选项和对应账单/统计中可见。
...
>
> 保留 Windows 开发、iPhone 优先、纯本地 IndexedDB、无账户/后端/追踪、整数日元和 JSON 备份兼容性。当前是奶油黄橙色猫咪手绘主题，本地含新增分类猫咪共 8 种图案（发布状态见第 0 节）；不要把多个图标尺寸说成多张独立插图。
>
